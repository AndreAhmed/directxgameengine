...
namespace Library
{
    ...
    Mesh::Mesh(Model& model, aiMesh& mesh, cGraphics *graphics)
        : mModel(model), mMaterial(nullptr), mGraphics(graphics), mName(mesh.mName.C_Str()), mVertices(), mNormals(), mTangents(), mBiNormals(), mTextureCoordinates(), mVertexColors(), mFaceCount(0), mIndices()
    {
    ...
    void Mesh::CreateIndexBuffer(ID3D11Buffer** indexBuffer)
    {
        ...
        if (FAILED(mGraphics->getDevice()->CreateBuffer(&indexBufferDesc, &indexSubResourceData, indexBuffer)))
        {
			throw cGameException(gameErrorNS::FATAL_ERROR, "ID3D11Device::CreateBuffer() failed."); 
        }
    }
}